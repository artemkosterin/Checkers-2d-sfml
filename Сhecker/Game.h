...
public:
	...
	void error_choise_checker(RenderWindow &_window, Event _event) {
		...
		while (1) {
			RectangleShape back = RectangleShape(Vector2f(220, 70));
			back.setFillColor(Color(245, 210, 175));
			back.setPosition(150, 215);
			Font font;
			font.loadFromFile("Font//bahnschrift.ttf");
			Text t = Text("", font, 25);
			t.setString(L"Необходимо есть");
			t.setFillColor(Color::Black);
			t.setPosition(155, 230);
			Text out = Text("", font, 9);
			out.setString(L"Нажмите любую клавишу, чтобы продолжить");
			out.setFillColor(Color::Black);
			out.setPosition(156, 255);
			_window.draw(back);
			_window.draw(t);
			_window.draw(out);
			_window.display();
	...
	bool end_game(RenderWindow &_window, Event _event) {
		...
		if (white == 0) {
				...
				font.loadFromFile("Font//bahnschrift.ttf");
				Text t = Text("", font, 40);
				t.setString(L"Черные выиграли");
				t.setFillColor(Color::Black);
				t.setPosition(78, 215);
				_window.draw(back);
				_window.draw(t);
		...
		if (black == 0) {
			...
			font.loadFromFile("Font//bahnschrift.ttf");
			Text t = Text("", font, 40);
			t.setString(L"Белые выиграли");
			t.setFillColor(Color::Black);
			t.setPosition(75, 215);
			_window.draw(back);
			_window.draw(t);
			if (_event.type == Event::Closed)
				_window.close();
